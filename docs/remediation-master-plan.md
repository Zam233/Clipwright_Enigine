...
## 轮 74：SRT 导入落轨修复（2026-09-08）

| # | 修复项 | 状态 |
|---|--------|------|
| 字幕 | SRT 导入只落 caption 轨（无则新建），不再复用已存在的 text 轨——旧实现把 `kind: 'caption'` 片段塞进 text 轨，PropertiesPanel 的 `trackKind === 'caption'` 级联判断失效，改样式只能逐条改 | ✅ |

回归：前端 tsc 0 错误 + vitest 405/405（+2 SRT 落轨）；后端无改动。

## 轮 75：未保存指示 + 批量整体平移（2026-09-08）

| # | 修复项 | 状态 |
|---|--------|------|
| 未保存指示 | `timelineStore.isDirty` 此前零消费者（自动保存用组件内 ref）——新增 `markSaved()` 并在保存成功后调用，状态栏显示「● 未保存」（warning 色）：5s 自动保存窗口内也能看到有未落盘更改 | ✅ |
| 批量平移 | 多选时属性面板新增「整体平移」秒数输入 + 应用：按最小起点钳制位移（保持相对间距），字幕整体挪 10s 不再需要逐条逐帧 nudge | ✅ |

回归：前端 tsc 0 错误 + vitest 413/413（+8：未保存 4 + 平移 4）；后端无改动。

## 轮 76：转场预览保真 + 占位标注（2026-09-08）

| # | 修复项 | 状态 |
|---|--------|------|
| 转场预览 | `slide`/`wipe` 从 alpha 渐隐改为几何转场：slide 进/出按窗口位移（fw→0 / 0→-fw），wipe 进/出按窗口擦除（可见宽 0→fw / fw→0）；`applyTransitionAlpha` 只保留 fade/dissolve/pixel_dissolve/glitch（旧实现所有几何转场都退化成淡变） | ✅ |
| 占位标注 | 动画/MG 片段的脉冲占位形加「未渲染预览」角标——占位形不是真实渲染结果，避免用户误当成品效果 | ✅ |

回归：前端 tsc 0 错误 + vitest 418/418（+5 几何转场）；后端无改动。

## 轮 77：修剪边缘吸附（2026-09-08）

| # | 修复项 | 状态 |
|---|--------|------|
| 修剪吸附 | trim-start/trim-end 接入与移动吸附同一套目标集/阈值（`collectSnapTargets` + `applySnap`）——旧实现 trim 分支显式 `snapX = null`，对齐相邻片段边界只能像素级微调。实现中修正了 `applySnap` 返回值语义（是增量 deltaTime 而非绝对时间） | ✅ |

回归：前端 tsc 0 错误 + vitest 420/420（+2 修剪吸附）；后端无改动。

# 帧艺 ClipWright 内容视频编排引擎 — Persona 系统设计

> **所属系统**：[帧艺 ClipWright AI 辅助视频创作系统](../README.md)
>
> Persona 系统是**内容视频编排引擎**的核心创新模块，定义四层复合的数字人格体系。Persona 将创作者风格参数化为可计算、可迁移、可版本控制的配置层，驱动 Agent 管线生成风格一致的视频输出。

---

## Persona 的三大组成部分

每个 Persona 由三大部分构成，覆盖从结构化参数到自然语言指令再到引用素材的完整表达：
...
三部分是**存储格式**，四层是**能力维度**。两者正交：

- YAML 参数层 ≈ 参数层
- Prompt 指令 → 影响所有四层的行为
- RAG 知识库 → 为示例层和嵌入层提供素材

---

## Persona 的类型体系

帧艺的 Persona 是一个**分层复合的数字人格包**。一个完整的 Persona 由四种类型的表达层叠加构成：
